...
export const createPaymentIntent = TryCatch(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if ( !amount) {
      return next(
        new ErrorHandler(`please enter the amount`, 400)
      );
    }
...
export const newCoupon = TryCatch(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!coupon || !amount) {
      return next(
        new ErrorHandler(`please enter all field for craeating coupon`, 400)
      );
    }
...
export const applyDiscount = TryCatch(
    ...
    if(!discount) return next(new ErrorHandler("no discount found invalid coupon code ", 400))
 ...
 if(!coupon)return next(new ErrorHandler(`coupon widh id-${req.params.id} not found `,404 ))
