...
export const getDashboardStats = TryCatch(async (req, res, next) => {
    ...
    if (myCache.has(key)) {
        ...
        const [thisMonthProducts, lastMonthProducts, thisMonthUsers, lastMonthUsers, thisMonthOrders, lastMonthOrders, productsCount, usersCount, totalOrders, lastSixMonthOrders, categories, femaleUserCount, latestTransactions] = await Promise.all([
        ...
        //     })
        // ])
        const cartegoryCount = await getInventories(categories, productsCount);
...
export const getPieCharts = TryCatch(async (req, res, next) => {
    ...
    if (myCache.has(key)) {
        ...
        const [processingOrder, shippedOrder, deliveredOrder, categories, productsCount, ourOfStock, allOrdersData, allUsersDataDob, adminUsers, customerUsers] = await Promise.all([
